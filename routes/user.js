...
router.post("/sign", function (req, res) {
  ...
  var sql = "insert into user  (USER_ID, USER_PASSWORD, USER_NAME) values (?,?,?)";
  var sql_cart = "insert into cart(USER_ID) values (?)";
      ...
      pool.getConnection((err, conn)=> {
        conn.query(sql, [req.body.id, body.password, body.name], 
            function(err, result){
                ...
                if(result){
                    conn.query(sql_cart, [req.body.id], 
                        function(err, result){
                            if(err){
                                console.log(err);
                                res.writeHead(200, { "Content-Type": "text/html; charset=utf-8;" });
...
router.post("/login", function (req, res) {
    ...
    pool.getConnection((err, conn) => {
      if (err) throw err;
      var sql = "SELECT * FROM user where USER_ID=? and USER_PASSWORD=?";
